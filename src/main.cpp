...
{
    bn::fixed speed = bn::fixed(1) + bn::fixed(rng.get_int(3));
    if (rng.get_int(2) == 0)
    {
        speed *= -1;
    }
...
public:
    ...
    void update()
    {
        ...
        else if (x < MIN_X) { x = MIN_X; x_speed *= -1; }

        if (y > MAX_Y) { y = MAX_Y; y_speed *= -1; }
        else if (y < MIN_Y) { y = MIN_Y; y_speed *= -1; }
